...
const OUTPUT_PATH = "./ETFEfficientFrontier.html";
...
async function readInfoText(path: string): Promise<string> {
  try {
    ...
    return normalizeInfoMarkdown(text) || "ETFEfficientFrontier";
  } catch (error) {
    if (error instanceof Deno.errors.NotFound) {
      return "ETFEfficientFrontier";
    }
...
function extractDocumentTitle(infoText: string): string {
  ...
  return title?.trim() || "ETFEfficientFrontier";
}
